...
## 抓取工具（已備，2026-06-13 補）

`tools/fetch-official-cards.mjs`（`npm run fetch:official`）：用 `itemsearch.php?p=N` 逐頁抓全量卡片（每頁 20 筆、空頁即停、sleep 300ms 禮貌延遲）→ 寫 `data/raw/official_cards.json`，供 `merge-official` 消費。含健全性檢查（缺 card_no／ID 重複／筆數過少即中止，不覆蓋既有檔），並印出新增/移除的卡號（增量回饋）。可重跑、全量覆蓋。
（M1 當時的抓取是一次性腳本未留存，本工具補回。2026-06-13 實測：364 列／296 唯一卡號，與既有檔一致。）
...
1. **抓取**（增量、可重跑、已存在跳過）：
   - `npm run fetch:official` → `data/raw/official_cards.json`（見上「抓取工具」）
