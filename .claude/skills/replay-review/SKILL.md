...
> **推理等級建議**（[使用者 2026-07-04] 採納 orchestrator 建議）：**triage 掃描＋戰報彙整＝中**（重活在工具，模型只讀取整理）；**單手決策深掘＋通則提煉＝高**（隱藏資訊 EV 推理、規則/判例正確性、教訓適用邊界）；**超高只留兩種例外**＝使用者與教練分析僵持不下的爭議手、多卡交互＋判例邊角的規則爭議。注意：推理等級治不了幻覺——接地契約的遵守優先於等級調整。
