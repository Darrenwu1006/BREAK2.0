...
// 依賴方向：engine.ts → effects.ts → (effect-helpers/deploy-legality/conditions/scripts) → (types, dsl)。
// [Claude 2026-07-16] 巨檔拆分：查詢工具/登場合法性/條件費用/特例腳本外移，本檔留佇列＋自由步驟＋解釋器，
// 並 re-export 舊符號（消費端 import 路徑不變）。
...
import { allGutsOf, baseParam, cardOf, charaAreaOf, charasOf, deployNames, effParam, effectDefOf, eventTimingsOf, gutsFor, log, matchEventAreaFilter, nameOf, normName, other, playTimingsOf, purgeModifiers, removeFromHand, topChara } from "./effect-helpers";
import { banHandAddActive, blockDeployMax, canDeployTo, centerBlockNegated, deployLegality } from "./deploy-legality";
import { costOps, costPayable, evalCond, isSkillInvalid, matchFilter } from "./conditions";
import { keywordActions, SCRIPTS } from "./scripts";
