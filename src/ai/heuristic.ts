...
export function heuristicAiDecision(db: CardDb, state: GameState): Decision {
  ...
  switch (pd.type) {
    ...
    case "mulligan": {
      // 事件卡在技能系統接入前無法使用 → 全部退回換抽
      const events = hand.filter((u) => db.get(state.cards[u]!)!.type === "EVENT");
      return { type: "mulligan", returnUids: events };
    }
    ...
    case "deploy-serve": {
      ...
      return { type: "deploy-serve", uid: best };
    }
    ...
    case "deploy-receive": {
      ...
      return { type: "deploy-receive", uid: cheapest };
    }
    ...
    case "deploy-toss": case "deploy-attack": {
      ...
      return { type: pd.type, uid: best } as Decision;
    }

    case "deploy-block": {
      const op = incomingOp(state, p);
      const ranked = bestBlockers(db, state, p);
      if (!ranked.length) return { type: "deploy-block", uids: null };
      ...
      return { type: "deploy-block", uids, center: uids[0]! };
    }
