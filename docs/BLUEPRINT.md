...
### M3 — 效果系統 ⬜
- [ ] 效果 DSL 設計（trigger/cost/condition/target/action/duration）
- [ ] 修正層系統（點數加減、設值、持續時間管理）
- [ ] 把現有卡池效果逐張轉成 DSL（分批，從主流卡開始）
- [ ] 判例 → 單元測試
- [ ] 特例卡腳本機制
...
- [ ] 第二版（M3 後）：自由步驟使用技能/事件卡的互動、ターン1/關鍵字顯示
...
- [ ] M3 效果系統後：技能使用的啟發式（何時開技能、Guts 管理）
- [ ] 強化：determinized MCTS（引擎已就緒：純資料狀態+內嵌 RNG）

### M6 — 牌組編輯器 ⬜
- [ ] 卡池瀏覽：篩選（學校/位置/點數/時機點/稀有度）、搜尋
- [ ] 牌組編輯：加卡減卡、合法性即時檢查（40張、事件≤8；無同名上限）、選卡面
- [ ] 存回 decks/ 資料夾（CSV 相容格式）、可直接拿去對戰
- [ ] 機翻校對流程：在卡片詳情中校對 translations.json 並解除「機翻待校」標記
